...
class Node:
    ...
    def __init__(self, data = Null, next = Null, dnext = Null):
        ...
        self.dnext = dnext          # 프리 리스트의 뒤쪽포인터 
...
class ArrayLinkedList:
    ...
    def delete_index(self, idx: int) -> None:
        ...
        if self.deleted == Null:            # 삭제 레코드는 존재하지 않음
            self.deleted = idx
            self.n[idx].dndext = Null       # idx를 프리 리스트의 맨 앞에 등록
        else:
            rec = self.deleted
    ...
    def remove(self, p: int) -> None:
        ...
        if self.head != Null:
            if p == self.head:
                self.remove_first()                 # p가 머리 노드면 머리 노드 삭제
            else:
                ptr = self.head
    ...
    def __iter__(self) -> ArrayLinekdListIterator:
        # 이터레이터 반환
        return ArrayLinekdListIterator(self.n, self.head)

class ArrayLinekdListIterator:
